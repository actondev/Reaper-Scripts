...
function module:getNextCursor()
    return math.min(string.len(self.data.text), self.data.cursor + 1)
end
...
function module:handle(c, mods)
    ...
    local actions = 
    {
        [Chars.CHAR.BACKSPACE] = self.handleBackspace,
        [Chars.CHAR.LEFT] = self.handleLeft,
        [Chars.CHAR.RIGHT] = self.handleRight,
        [Chars.CHAR.DELETE] = self.handeDelete
    }
    if actions[c] then
        actions[c](self)
    end
