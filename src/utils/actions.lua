...
function module.search(section, query, limit)
    ...
    for i, action in ipairs(commandCache[sectionName]) do
        actionLowercase = string.lower(action.name)
        -- Log.debug("action " .. actionLowercase)
        local match = true
        -- all words in the query string must match the action name
        for token in string.gmatch(query, "[^%s]+") do
            -- Log.debug("token " .. token)
            local findI, findJ = string.find(actionLowercase, token:lower(), 1, true)
            -- Log.debug("i j  " .. tostring(findI) .. "  " .. tostring(findJ))
            if findI == nil then
                match = false
                break
            end
